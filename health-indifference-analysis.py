# -*- coding: utf-8 -*-
"""Health Indifference (Presentation_GOLEM+DAGMA+CV)
...
# -*- coding: utf-8 -*-
"""
JASTIS Health Indifference Research - 5-fold CV version
...
# ========================================
# 2. Library Imports and Settings
# ========================================
import pandas as pd
import numpy as np
...
class HealthIndifferenceCalculator:
    ...
    def calculate_score(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if df['HealthIndifferenceScore'].notna().any():
            ...
            # Create binary variable as well
...
class MissingDataHandler:
    ...
    def analyze_missing_patterns(self, df: pd.DataFrame) -> Dict:
        ...
        if len(missing_summary) > 0:
            ...
            plt.tight_layout()
            plt.savefig('missing_pattern.png', dpi=300)
            plt.close()
    ...
    def perform_mice(self, df: pd.DataFrame, vars_to_impute: List[str]) -> List[pd.DataFrame]:
        ...
        print(f"Performing multiple imputation ({self.n_imputations} times)...")
...
class StatisticalAnalyzer:
    ...
    def is_count_variable(var_name: str) -> bool:
        """Determine if a variable is a count variable based on its name"""
    ...
    def create_table1(df: pd.DataFrame) -> pd.DataFrame:
        """Table 1: Create participant characteristics"""
        results = []
...
class CausalInferenceAnalyzer:
    ...
    def __init__(self, random_state: int = 12345, hi_var_type: str = 'continuous', use_cv: bool = True, n_folds: int = 5):
        ...
        random_state : int
            Random seed
        hi_var_type : str
            'continuous': HealthIndifferenceScore (13-52 continuous values)
            'binary': High_HI (0/1 binary variable)
        use_cv : bool
        ...
        # Logical constraints (unchanged)
        self.logical_constraints = [
            # COVID infection → vaccination (logically reverse)
            ('COVID19_infection_within_1year_23', 'COVID19_vaccinated_23'),
            ('COVID19_infection_within_1year_23', 'Influenza_vaccinated_23'),

            # Constraints within same time point in 2021 to invariant variables
            # → Male
            ('Lack_social_support', 'Male'),
            ('Current_smoking', 'Male'),
            ('No_health_checkup', 'Male'),
            ('Low_income', 'Male'),
            ('Living_alone', 'Male'),
            ('Poor_self_rated_health', 'Male'),
            ('Heavy_drinking', 'Male'),
            ('Low_physical_activity', 'Male'),
            ('BMI_21', 'Male'),
            ('Disease_count_21', 'Male'),
            ('Symptom_count_21', 'Male'),

            # → Age_21
            ('Lack_social_support', 'Age_21'),
            ('Current_smoking', 'Age_21'),
            ('No_health_checkup', 'Age_21'),
            ('Low_income', 'Age_21'),
            ('Living_alone', 'Age_21'),
            ('Poor_self_rated_health', 'Age_21'),
            ('Heavy_drinking', 'Age_21'),
            ('Low_physical_activity', 'Age_21'),
            ('BMI_21', 'Age_21'),
            ('Disease_count_21', 'Age_21'),
            ('Symptom_count_21', 'Age_21'),

            # → Low_education
            ('Lack_social_support', 'Low_education'),
            ('Current_smoking', 'Low_education'),
            ('No_health_checkup', 'Low_education'),
            ('Low_income', 'Low_education'),
            ('Living_alone', 'Low_education'),
            ('Poor_self_rated_health', 'Low_education'),
            ('Heavy_drinking', 'Low_education'),
            ('Low_physical_activity', 'Low_education'),
            ('BMI_21', 'Low_education'),
            ('Disease_count_21', 'Low_education'),
            ('Symptom_count_21', 'Low_education'),

            # Mutual causation between invariant variables
            ('Male', 'Age_21'),
            ('Age_21', 'Male'),
            ('Male', 'Low_education'),
            ('Low_education', 'Male'),
            ('Age_21', 'Low_education'),
            ('Low_education', 'Age_21'),

            # From HI variables to invariant variables
            ('HealthIndifferenceScore', 'Male'),
            ('HealthIndifferenceScore', 'Age_21'),
            ('HealthIndifferenceScore', 'Low_education'),
            ('High_HI', 'Male'),
            ('High_HI', 'Age_21'),
            ('High_HI', 'Low_education'),

            # From 2023 diseases to invariant variables
            ('Hypertension_23_current', 'Male'),
            ('Diabetes_23_current', 'Male'),
            ('Dyslipidemia_23_current', 'Male'),
            ('Depression_23_current', 'Male'),
            ('Periodontitis_23_current', 'Male'),
            ('Asthma_23_current', 'Male'),
            ('ChronicObstructivePulmonaryDisease_23_current', 'Male'),
            ('ChronicKidneyDisease_23_current', 'Male'),
            ('DentalCaries_23_current', 'Male'),
            ('AnginaOrMI_23_current', 'Male'),
            ('Stroke_23_current', 'Male'),
            ('ChronicLiverDisease_23_current', 'Male'),
            ('Cancer_23_current', 'Male'),
            ('ChronicPain_23_current', 'Male'),
            ('COVID19_infection_within_1year_23', 'Male'),
            ('HospitalizationPastYear_23', 'Male'),

            ('Hypertension_23_current', 'Age_21'),
            ('Diabetes_23_current', 'Age_21'),
            ('Dyslipidemia_23_current', 'Age_21'),
            ('Depression_23_current', 'Age_21'),
            ('Periodontitis_23_current', 'Age_21'),
            ('Asthma_23_current', 'Age_21'),
            ('ChronicObstructivePulmonaryDisease_23_current', 'Age_21'),
            ('ChronicKidneyDisease_23_current', 'Age_21'),
            ('DentalCaries_23_current', 'Age_21'),
            ('AnginaOrMI_23_current', 'Age_21'),
            ('Stroke_23_current', 'Age_21'),
            ('ChronicLiverDisease_23_current', 'Age_21'),
            ('Cancer_23_current', 'Age_21'),
            ('ChronicPain_23_current', 'Age_21'),
            ('COVID19_infection_within_1year_23', 'Age_21'),
            ('HospitalizationPastYear_23', 'Age_21'),

            ('Hypertension_23_current', 'Low_education'),
            ('Diabetes_23_current', 'Low_education'),
            ('Dyslipidemia_23_current', 'Low_education'),
            ('Depression_23_current', 'Low_education'),
            ('Periodontitis_23_current', 'Low_education'),
            ('Asthma_23_current', 'Low_education'),
            ('ChronicObstructivePulmonaryDisease_23_current', 'Low_education'),
            ('ChronicKidneyDisease_23_current', 'Low_education'),
            ('DentalCaries_23_current', 'Low_education'),
            ('AnginaOrMI_23_current', 'Low_education'),
            ('Stroke_23_current', 'Low_education'),
            ('ChronicLiverDisease_23_current', 'Low_education'),
            ('Cancer_23_current', 'Low_education'),
            ('ChronicPain_23_current', 'Low_education'),
            ('COVID19_infection_within_1year_23', 'Low_education'),
            ('HospitalizationPastYear_23', 'Low_education'),

            # From 2023 symptoms to invariant variables
            ('Fatigue_23_moderate', 'Male'),
            ('SleepDisorder_23_moderate', 'Male'),
            ('BackPain_23_moderate', 'Male'),
            ('DigestiveDisorder_23_moderate', 'Male'),
            ('Headache_23_moderate', 'Male'),
            ('JointPain_23_moderate', 'Male'),
            ('ConcentrationDecline_23_moderate', 'Male'),
            ('MemoryImpairment_23_moderate', 'Male'),
            ('ChestPain_23_moderate', 'Male'),
            ('Dyspnea_23_moderate', 'Male'),
            ('Dizziness_23_moderate', 'Male'),
            ('ReducedLibido_23_moderate', 'Male'),
            ('Cough_23_moderate', 'Male'),
            ('Fever_23_moderate', 'Male'),

            ('Fatigue_23_moderate', 'Age_21'),
            ('SleepDisorder_23_moderate', 'Age_21'),
            ('BackPain_23_moderate', 'Age_21'),
            ('DigestiveDisorder_23_moderate', 'Age_21'),
            ('Headache_23_moderate', 'Age_21'),
            ('JointPain_23_moderate', 'Age_21'),
            ('ConcentrationDecline_23_moderate', 'Age_21'),
            ('MemoryImpairment_23_moderate', 'Age_21'),
            ('ChestPain_23_moderate', 'Age_21'),
            ('Dyspnea_23_moderate', 'Age_21'),
            ('Dizziness_23_moderate', 'Age_21'),
            ('ReducedLibido_23_moderate', 'Age_21'),
            ('Cough_23_moderate', 'Age_21'),
            ('Fever_23_moderate', 'Age_21'),

            ('Fatigue_23_moderate', 'Low_education'),
            ('SleepDisorder_23_moderate', 'Low_education'),
            ('BackPain_23_moderate', 'Low_education'),
            ('DigestiveDisorder_23_moderate', 'Low_education'),
            ('Headache_23_moderate', 'Low_education'),
            ('JointPain_23_moderate', 'Low_education'),
            ('ConcentrationDecline_23_moderate', 'Low_education'),
            ('MemoryImpairment_23_moderate', 'Low_education'),
            ('ChestPain_23_moderate', 'Low_education'),
            ('Dyspnea_23_moderate', 'Low_education'),
            ('Dizziness_23_moderate', 'Low_education'),
            ('ReducedLibido_23_moderate', 'Low_education'),
            ('Cough_23_moderate', 'Low_education'),
            ('Fever_23_moderate', 'Low_education'),

            # From vaccination to invariant variables
            ('COVID19_vaccinated_23', 'Male'),
            ('COVID19_vaccinated_23', 'Age_21'),
            ('COVID19_vaccinated_23', 'Low_education'),
            ('Influenza_vaccinated_23', 'Male'),
            ('Influenza_vaccinated_23', 'Age_21'),
            ('Influenza_vaccinated_23', 'Low_education'),
        ]
    ...
    def prepare_causal_data(self, df: pd.DataFrame, variable_subset: str = 'all') -> Tuple[np.ndarray, List[str]]:
        """Prepare data for causal inference (supports HI variable type)"""
        print(f"\nPreprocessing policy ({variable_subset} group)：")
        print("- Binary variables (0/1): No standardization (preserve interpretability)")
        print("- Continuous variables: Standardize with RobustScaler (robust to outliers)")
        print("- Count variables: Standardize with RobustScaler")
        ...
        # Add HI variable if continuous and not in scale list
    ...
    def classify_vars_by_year(self, var_names: List[str]) -> Dict[str, List[int]]:
        """Classify variables by year (supports HI variable type)"""
    ...
    def create_prior_knowledge_matrix(self, var_names: List[str], strictness: str = 'adaptive') -> Tuple[np.ndarray, Dict]:
        ...
        # Time series constraints (prohibit future→past)
        constraint_count = 0
        ...
        print(f"- Time series constraints: {constraint_count}")
        ...
        if strictness == 'strict':
            ...
            # Constraints only for critical immutable variables
            ...
            for target_var in critical_immutable:
                if target_var in var_names:
                    ...
                    # Prohibit causation only from variables of the same year
            ...
            print("- No additional constraints (time series constraints only)")
        ...
        if cant_be_effect_count >= 3:
            print("⚠️  Warning: 3 or more variables cannot be effects, DirectLiNGAM is likely to fail")
    ...
    def filter_violations(self, causal_matrix: np.ndarray, var_names: List[str],
        ...
        # Time series violations (remove future→past)
        # causal_matrix[i,j] is the coefficient j→i

        # Remove causation 2023→2021/2022
        for i in vars_by_year['2021'] + vars_by_year['2022']:
            for j in vars_by_year['2023']:
        ...
        if total_violations > 0 and method_name:
            print(f"\n{method_name} - Removed constraint violations:")
            if violations['temporal'] > 0:
                print(f"  - Time series violations: {violations['temporal']}")
    ...
    def _aggregate_cv_results(self, fold_results: List[Dict], method_name: str, variable_subset: str) -> Dict:
        ...
        significant_links = {}
        threshold_folds = max(3, int(self.n_folds * 0.6))  # Detected in 60%+ of folds
        ...
        return {
            'causal_matrix': None,  # Don't retain individual matrices during CV
    ...
    def _run_directlingam_single(self, data: np.ndarray, var_names: List[str], variable_subset: str) -> Dict:
        ...
        try:
            ...
            # Try from strictest constraints
    ...
    def _run_golem_single(self, data: np.ndarray, var_names: List[str], variable_subset: str) -> Dict:
        ...
        try:
            ...
            # Initialize time series violating elements to 0
            for i in vars_by_year['2021'] + vars_by_year['2022']:
                for j in vars_by_year['2023']:
                    B_init[i, j] = 0
            for i in vars_by_year['2021']:
            ...
            params = {'lr': 1e-3, 'lambda_1': 2e-2, 'lambda_2': 5.0, 'num_iter': 100000} # lambda_1=0.02, lambda_2=5.0, learning_rate=0.001, num_iter=100000.0
    ...
    def _run_corl_single(self, data: np.ndarray, var_names: List[str], variable_subset: str) -> Dict:
        ...
        try:
            ...
            print(f"Variable list ({len(var_names)}): {var_names[:5]}...")  # Show only first 5

            # Ensure data is float32 and C-contiguous array
            ...
            params = {'embed_dim': 256, 'encoder_blocks': 4, 'iteration': 1000}

            # Initialize CORL (remove dtype parameter)
            corl = CORL(
                batch_size=64,
                input_dim=100,
    ...
    def _run_dagma_single(self, data: np.ndarray, var_names: List[str], variable_subset: str) -> Dict:
        ...
        try:
            ...
            params = {'T': 4, 'max_iter': 8000, 'warm_iter': 5000} # 8000/5000

            eq_model = DagmaMLP(
                dims=[n_variables, 10, 10, 1],
                bias=True,
                dtype=torch.float32
            )
    ...
    def run_directlingam_analysis(self, df: pd.DataFrame, variable_subset: str = 'all') -> Dict:
        """DirectLiNGAM analysis (CV supported)"""
    ...
    def run_dagma_analysis(self, df: pd.DataFrame, variable_subset: str = 'all') -> Dict:
        """DAGMA analysis (CV supported)"""
        return self.run_method_cv(df, 'DAGMA', variable_subset)

    def integrate_results(self, df: pd.DataFrame) -> pd.DataFrame:
        """Integrate results from 4 methods (CV supported version)"""
        print("\nIntegrating causal inference results...")

        if self.use_cv:
            print(f"CV settings: {self.n_folds}-fold CV, threshold: {max(3, int(self.n_folds * 0.6))}+ folds for significance")
        ...
        for group in groups:
            ...
            print(f"Executing causal inference for {group_display_name} group...")
            print(f"{'='*60}")
        ...
        print(f"\nIntegrated results:")
        print(f"- Total links detected: {len(all_links)}")
...
class LongitudinalAnalyzer:
    """Execute longitudinal analysis (mutual adjustment model)"""

    @staticmethod
    def run_logistic_regression(df: pd.DataFrame, outcome: str,
                              exposure: str, covariates: List[str]) -> Dict:
        """Logistic regression analysis (single exposure variable)"""
    ...
    @staticmethod
    def run_logistic_regression_mutual_adjustment(df: pd.DataFrame,
                                                 outcome: str,
                                                 determinants: List[str]) -> Dict:
        """Mutual adjustment logistic regression (for determinant analysis)"""
        ...
        try:
            ...
            for det in determinants:
                if det in model.params.index:
                    or_val = np.exp(model.params[det])
    ...
    def create_table2_determinants(df: pd.DataFrame) -> pd.DataFrame:
        """Table 2: Determinants of Health Indifference (mutual adjustment model + VIF)"""
        results = []

        print("\nAnalyzing determinants of health indifference (mutual adjustment model)...")
        ...
        if 'HI_category' in df.columns:
            ...
            # Execute mutual adjustment model
            model_result = LongitudinalAnalyzer.run_logistic_regression_mutual_adjustment(
                    ...
                    if var in model_result['results']:
                        ...
                        results.append({
                            ...
                            'Mutually Adjusted OR (95% CI)':
                                f"{result['or']:.2f} ({result['ci_lower']:.2f}-{result['ci_upper']:.2f})",
    ...
    def create_table3_outcomes(df: pd.DataFrame) -> pd.DataFrame:
        ...
        print("\nAnalyzing health outcomes (adjusted for all determinants)...")

        if 'HI_category' in df.columns:
            df['High_HI'] = (df['HI_category'] == 'High HI').astype(int)

            # All 2021 determinants (used as covariates)
            ...
            for outcome, label in all_outcomes:
                if outcome in df.columns:
                    result = LongitudinalAnalyzer.run_logistic_regression(
...
class Visualizer:
    ...
    @staticmethod
    def create_participant_flowchart(df: pd.DataFrame, output_path: str = 'figure1_participant_flow.png'):
    ...
    def create_forest_plot(table_df: pd.DataFrame, plot_type: str, output_path: str):
        """Create forest plot (for Table 2 or Table 3)"""
        fig, ax = plt.subplots(figsize=(10, 12))
        ...
        if table_df is not None and not table_df.empty:
            for _, row in table_df.iterrows():
                try:
                    or_col_name = None
                    if 'Mutually Adjusted OR (95% CI)' in row:
                        or_col_name = 'Mutually Adjusted OR (95% CI)'
                    elif 'Adjusted OR (95% CI)' in row:
                        or_col_name = 'Adjusted OR (95% CI)'

                    if or_col_name:
                        or_ci = row[or_col_name]
                        or_match = re.match(r'([\d.]+)\s*\(([\d.]+)-([\d.]+)\)', or_ci)

                        if or_match:
                            or_val = float(or_match.group(1))
                            ci_low = float(or_match.group(2))
                            ci_high = float(or_match.group(3))
                    ...
                    print(f"Warning: Error extracting OR value for {row['Variable']}: {e}")
                    continue

        if not outcomes:
            print(f"Warning: No data available for {plot_type} forest plot")
            ax.text(0.5, 0.5, 'No data available',
                    ha='center', va='center', transform=ax.transAxes, fontsize=14)
            ax.set_xlim(0, 1)
            ax.set_ylim(0, 1)
        else:
            outcomes.sort(key=lambda x: x['or'], reverse=True)
            ...
            for i, outcome in enumerate(outcomes):
                ...
                try:
                    ...
                    color = 'gray'
                ax.plot(outcome['or'], i, 'o', color=color, markersize=8)

                ci_text = f"{outcome['or']:.2f} ({outcome['ci_low']:.2f}-{outcome['ci_high']:.2f})"
            ...
            all_values = []
            for o in outcomes:
                all_values.extend([o['ci_low'], o['ci_high']])
            if all_values:
                x_min = min(0.5, min(all_values) * 0.9)
                x_max = max(2.0, max(all_values) * 1.1) + 0.5
                ax.set_xlim(x_min, x_max)

            ax.grid(True, alpha=0.3)

        if plot_type == 'determinants':
            title = 'Figure 2A. Forest Plot: Determinants of Health Indifference'
            subtitle = '(Mutually adjusted odds ratios for High HI vs Low HI)'
        else:
            title = 'Figure 2B. Forest Plot: Health Outcomes Associated with Health Indifference'
            subtitle = '(Adjusted for baseline characteristics)'
    ...
    def create_forest_plots(table2_df: pd.DataFrame, table3_df: pd.DataFrame):
        ...
        if table2_df is not None and not table2_df.empty:
            Visualizer.create_forest_plot(
                table2_df,
                'determinants',
                'figure2a_forest_plot_determinants.png'
            )
        ...
        if table3_df is not None and not table3_df.empty:
            Visualizer.create_forest_plot(
                table3_df,
                'outcomes',
                'figure2b_forest_plot_outcomes.png'
            )

    @staticmethod
    def create_causal_dag(causal_results: Dict, output_path: str = 'figure3_causal_dag.png'):
        """Figure 3: Split DAG based on causal inference results into two (supports HI variable type)"""
        ...
        def create_dag_subset(edges_with_details: Dict, all_vars: set,
            ...
            # Classify variables by time point and type
            vars_2021 = []
            vars_2022 = []
            vars_2023_target = []
            vars_2023_other = []
            ...
            # Select only edges related to this subset
            relevant_edges = {}
            ...
            # Edge drawing (color-blind friendly colors)
            base_colors = {
                'All four': '#D55E00',
            ...
            for edge, details in relevant_edges.items():
                if edge[0] in display_nodes and edge[1] in display_nodes:
                    ...
                    # Determine color and style based on HI relation
                    if base_color_key:
                        if hi_related:
            ...
            # Label according to HI variable type
            ...
            if subset_type == 'disease_infection':
                save_path = output_path.replace('.png', f'_disease_infection{cv_suffix}.png')
            else:
                save_path = output_path.replace('.png', f'_symptoms{cv_suffix}.png')
        ...
        if causal_results and 'integrated' in causal_results:
            ...
            for _, row in integrated_df.iterrows():
                ...
                if ' → ' in path:
                    ...
                    # Determine if HI-related (supports both HI variable types)
        ...
        # Create two figures
        print("Creating Figure 3 split into two (execution results by group)...")
        ...
        print(f"Created Figure 3 (Disease/Infection) and Figure 4 (Symptoms){cv_info}")

    @staticmethod
    def create_score_distribution(df: pd.DataFrame, output_path: str = 'supp_figure1_distribution.png'):
        ...
        plt.title(f'Supplementary Figure 1. Distribution of Health Indifference Scores',
    ...
    @staticmethod
    def create_causal_inference_details(causal_results: Dict, output_path: str = 'supp_figure2_causal_details.png'):
        """Supplementary Figure 2: Detailed visualization of causal inference results (supports HI variable type and CV)"""
        ...
        # 4. Table of major causal paths (with CV info)
        ax4 = axes[1, 1]
        ax4.axis('off')

        # Prioritize HI-related paths (supports both HI variable types)
        ...
        if len(hi_related) > 0:
            ...
            for _, row in display_paths.iterrows():
                path = row['Path']
                # Shorten path (supports HI variable type)
            ...
            # Display HI variable type and CV info
            hi_var_desc = 'High HI (Binary)' if hi_var_type == 'binary' else 'HI Score (Continuous)'
        ...
        plt.suptitle(f'Supplementary Figure 2. Causal Inference Analysis Details{cv_info_str}\n(HI Variable: {hi_var_type})',
...
def calculate_e_value(or_value: float, ci_lower: float = None, ci_upper: float = None) -> Tuple[float, float, float]:
    """Function to calculate E-value (supports both upper and lower bounds of confidence interval)"""
    ...
    # E-value for lower bound of confidence interval
    ...
    # E-value for upper bound of confidence interval
...
class JASTISAnalysisPipeline:
    """Main analysis pipeline (HI variable type selectable, CV supported)"""
    ...
    def run_complete_analysis(self):
        ...
        print("="*60)
        print("JASTIS Health Indifference Research - Analysis Pipeline (5-fold CV version)")
        print("="*60)
        ...
        print(f"Cross-Validation: {'5-fold CV' if self.use_cv else 'Single run'}")
        if self.use_cv:
            print("Significance determination: Accept edges detected in 3+ folds")
        print("Statistical methods: t-test, Brunner-Munzel test, Chi-square test")
        print("="*60)
        ...
        print("\n[Step 6] Longitudinal analysis (mutual adjustment model)...")
        ...
        if GCASTLE_AVAILABLE or DAGMA_AVAILABLE:
            ...
            if not causal_results.empty:
                print("\nSupplementary Table 1. Causal Inference Results")
                if self.use_cv:
                    print("(5-fold CV: Edges detected in ≥3 folds)")
                print("="*120)

                # Add number of detection methods to each row
                ...
                # HI-related statistics (supports both HI variable types)
    ...
    def calculate_e_values(self):
        ...
        if 'table3' in self.results and not self.results['table3'].empty:
            ...
            for _, row in outcomes_data.iterrows():
                try:
                    or_ci_str = row['Adjusted OR (95% CI)']
                    or_match = re.match(r'([\d.]+)\s*\(([\d.]+)-([\d.]+)\)', or_ci_str)
                    ...
                    if or_match:
                        ...
                        e_val, e_val_ci_lower, e_val_ci_upper = calculate_e_value(or_val, ci_low, ci_high)
                        ...
                        if e_val_ci_upper == float('inf'):
                            print(f"{row['Variable']}: OR = {or_val:.2f} ({ci_low:.2f}-{ci_high:.2f}), "
                                  f"E-value = {e_val:.2f} (CI: {e_val_ci_lower:.2f}-∞)")
                        else:
                            print(f"{row['Variable']}: OR = {or_val:.2f} ({ci_low:.2f}-{ci_high:.2f}), "
        ...
        if e_values:
            ...
            self.results['e_values'] = self.results['e_values'].sort_values('OR', ascending=False)
        else:
    ...
    def save_results(self):
        ...
        if DRIVE_MOUNTED and DRIVE_OUTPUT_DIR:
            ...
            if os.path.exists('missing_pattern.png'):
                shutil.copy2('missing_pattern.png', os.path.join(DRIVE_OUTPUT_DIR, 'missing_pattern.png'))

        print("\nResults saved to the following files:")
        print(f"- {excel_path}")
        print(f"- results/analysis_summary{hi_var_suffix}{cv_suffix}.txt")
        print("- Various figures (figure*.png, supp_figure*.png)")
    ...
    def create_summary_report(self):
        ...
        with open(f'results/analysis_summary{hi_var_suffix}{cv_suffix}.txt', 'w', encoding='utf-8') as f:
            f.write("JASTIS Health Indifference Research - Analysis Summary (5-fold CV version)\n")
            f.write("=" * 60 + "\n\n")
            f.write(f"Analysis date/time: {datetime.now().strftime('%Y-%m-%d %H:%M')}\n")
            f.write(f"Data file: {self.data_path}\n")
            f.write(f"Total sample size: {len(self.df):,}\n")
            f.write(f"HI variable type: {self.hi_var_type}\n")
            f.write(f"Cross-Validation: {'5-fold CV' if self.use_cv else 'Single run'}\n")
            ...
            if self.use_cv:
                f.write("Significance determination: Accept edges detected in 3+ folds\n")
            ...
            if 'table2' in self.results and not self.results['table2'].empty:
                ...
                if significant_determinants:
                    significant_determinants.sort(
                        key=lambda x: float(x['Mutually Adjusted OR (95% CI)'].split(' ')[0]),
                        reverse=True
                    )

                    for i, row in enumerate(significant_determinants[:5]):
                        f.write(f"   - {row['Variable']}: OR = {row['Mutually Adjusted OR (95% CI)']} (p {row['p-value']})")
                        if 'VIF' in row:
            ...
            if 'table3' in self.results and not self.results['table3'].empty:
                ...
                if significant_outcomes:
                    significant_outcomes.sort(
                        key=lambda x: float(x['Adjusted OR (95% CI)'].split(' ')[0]),
                        reverse=True
                    )

                    for i, row in enumerate(significant_outcomes[:5]):
                        f.write(f"   - {row['Variable']}: OR = {row['Adjusted OR (95% CI)']} (p {row['p-value']})\n")
                else:
...
def main(hi_var_type: str = 'continuous', use_cv: bool = True):
    ...
    if not os.path.exists(data_path):
        ...
        if IN_COLAB:
            print("\nTo upload file in Google Colab:")
...
if __name__ == "__main__":
    ...
    if IN_JUPYTER:
        ...
        print("1. continuous (HealthIndifferenceScore: 13-52 continuous values)")
        print("2. binary (High_HI: 0/1 binary variable)")
        ...
        print("2. No (Single run) - Fast, about 10-20 minutes")

        cv_choice = input("Selection (1 or 2): ").strip()
        use_cv = (cv_choice == '1')
        ...
        parser = argparse.ArgumentParser(description='JASTIS Health Indifference Research - Causal Inference Analysis')
        parser.add_argument('--hi_var_type', type=str, default='continuous',
                            choices=['continuous', 'binary'],
                            help='HI variable type (continuous: continuous values, binary: binary)')
        parser.add_argument('--use_cv', action='store_true', default=False,
                            help='Use 5-fold CV')
        parser.add_argument('--no_cv', dest='use_cv', action='store_false',
                            help='Do not use CV (single run)')
    ...
    print("\nAnalysis settings:")
    print(f"- HI variable type: {hi_var_type}")
    print(f"- Cross-Validation: {'5-fold CV' if use_cv else 'Single run'}")
...
"""Setting Normalize=True in CORL parameters makes CORL unusable (
...
Since CORL is reinforcement learning based, smaller batch sizes are faster but less accurate→batch_size=1: Fastest but accuracy drops significantly
batch_size=8-16: Good balance for small problems
batch_size=32-48: Generally recommended (sweet spot for speed and accuracy)
batch_size≥64: Accuracy plateaus, speed is slow

The trade-off between "speed" and "accuracy" is particularly pronounced in reinforcement learning, so it's important to adjust according to the importance of the problem

On the other hand, DAG-GNN is gradient-based, so larger batch sizes are better as capacity allows

Recommended order and expected performance by hardware
...
if torch.cuda.is_available():
    ...
    # Optimal settings according to GPU
    if "A100" in gpu_name:
        batch_size = 512
        hidden_size = 128
    elif "L4" in gpu_name:
...
if torch.cuda.is_available():
    # Enable mixed precision operations (especially effective on A100, L4)
    torch.backends.cudnn.benchmark = True
    torch.backends.cuda.matmul.allow_tf32 = True  # Valid on A100
Why DAG-GNN is inherently slow
DAG-GNN computation bottlenecks:

DAG constraint checking: k_max_iter iterations of optimization
